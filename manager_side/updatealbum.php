...
$idSong = $_GET['idSong'];
$querySong = "SELECT * FROM `Album` WHERE Id = $idSong";
...
?>
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Chỉnh sửa album</h1>
                    </div>
                        ...
                        <div class="col-lg-12 mb-10">
                            <div class="card shadow mb-9">
                                <div class="card-header py-4">
                                    <h6 class="m-1 font-weight-bolder text-primary text-center">Chỉnh sửa album</h6>
                                </div>
                                <div class="card-body">
                                    <form class="user" method="post" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="tenal">Tên album:</label>
                                            <input type="text" id="tenal" name="tenal" class="form-control" value = "<?php echo $songInfo['Title'] ?>">
                                        </div>

                                        <div class="form-group">
                                            <label for="type">Thể loại album</label>
                                            <select id="type" name="type" class="form-control" >
                                                <option value="EP">Đĩa mở rộng</option>
                                                <option value="album">ALbum</option>
                                                <option value="playlist">Tuyển tập</option>
                                                <option value="single">Đĩa đơn</option>
                                            </select>
                                        </div>
...
?>
                                        ...
                                        <div class="form-group">
                                            <label for="avatarURI">Bìa album</label>
                                            <input type="file" id="avatarURI" name="avatarURI" class="form-control-file">
                                        </div>
                                        <div class="form-group">
                                            <label for="date">Ngày phát hành album</label>
                                            <input type="date" id="date" name="date" class="form-control">
                                        </div>
                                        <div class="form-group">
                                            <label for="song">Danh sách bài hát</label>
                                            <select multiple="multiple" class="form-control" id="song" name="song[]">
                                                <?php
                                                require_once('dbconfig.php');
                                                require_once('model.php');
                                                ...
                                                while ($row = mysqli_fetch_assoc($sql)) {
                                                    $songs = $row['Id'];
                                                    $querySA = "SELECT * FROM `SongAlbum` WHERE `AlbumId` = '$idSong' AND `SongId` = '$songs'";
                                                    $sqlSA = mysqli_query($dbconn, $querySA);
                                                    $numrow = mysqli_num_rows($sqlSA);
                                                    if ($numrow == 1){
...
require_once('model.php');
    ...
    $idArtist = "";
        $date = "";
        $avatarURI = $songInfo['CoverURI'];
    if (isset($_POST['avatarURI'])) {
        $filepath = "/img/album/";
        if (!is_dir($file_path)) {
        ...
        if (move_uploaded_file($_FILES['avatarURI']['tmp_name'], $file_path)) {
            ...
            header('Location:themalbum.php?');
        }
    }
        if (isset($_POST['tenal'])) {
            $tenal = $_POST['tenal'];
        }
        if (isset($_POST['type'])) {
            $type = $_POST['type'];
    ...
    $date = strtotime($_POST['date']);
        ...
        $query1 = "UPDATE `Album` SET `ArtistId`='$idArtist',`Title`='$tenal',`CoverURI`='$avatarURI',`DateCreated`='$date',`Type`='$type' WHERE `Id` = '$idSong'";
        $sql = mysqli_query($dbconn, $query1);

    $query5 = "DELETE FROM `SongAlbum` WHERE `AlbumId` = '$idSong'";
    mysqli_query($dbconn, $query5);

    foreach ($_POST['song'] as $song) {
        $query3 = "INSERT INTO `SongAlbum`(`AlbumId`, `SongId`) VALUES ('$idSong','$song')";
