...
while ($row = mysqli_fetch_assoc($sql)) {
    $gerneId = $row['Id'];
    $query1="SELECT Count(SongGenre.SongId) as tong FROM `SongGenre` WHERE `GenreId` = '$gerneId'";
    $sql2 = mysqli_query($dbconn, $query1);
    $row2 = mysqli_fetch_assoc($sql2);
    echo '<tr>';
    echo "<td>{$row['Name']}</td>";
    echo "<td>{$row['Description']}</td>";
    echo "<td>{$row2[0]}</td>";
    echo "<td><a class='btn btn-success' href='updategenre.php?idSong=".$row['Id']."'> Chỉnh sửa</a></td>";
    echo '</tr>';
}
?>
