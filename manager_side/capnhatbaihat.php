            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Danh sách bài hát</h1>
                    </div>
...
$query = "SELECT C.song , C.Title, C.Genre, Artist.Name AS Artist, C.NumberOfStream, C.DateCreated FROM (SELECT B.songID as song, B.Title, B.Genre, B.NumberOfStream, B.DateCreated, SongArtist.ArtistId FROM (SELECT A.songID, A.Title, Genre.Name as Genre, A.NumberOfStream, A.DateCreated FROM (SELECT Song.Id AS songID, Song.Title, SongGenre.GenreId, Song.NumberOfStream, Song.DateCreated FROM Song JOIN SongGenre WHERE SongGenre.SongId = Song.Id) AS A JOIN Genre WHERE Genre.Id = A.GenreId) AS B JOIN SongArtist WHERE B.songId = SongArtist.SongId) as C JOIN Artist WHERE Artist.Id = C.ArtistId";
...
while($row = mysqli_fetch_assoc($sql)){
    ...
    echo "<td>{$row['NumberOfStream']}</td>";
    echo "<td>{$row['DateCreated']}</td>";
    echo "<td><a class='btn btn-success ico' href='update.php?idSong=".$row['song']."'> Chỉnh sửa</a></td>";
    echo '</tr>';
}
?>
